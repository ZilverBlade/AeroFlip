	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			if (IsWindowVisible(hWnd))
			{
				...
				if (!bAltHeld)
				{
					if (!g_bIsDismissing) g_bIsDismissing = TRUE;
				...
				if (g_bWindowListDirty)
				{
					...
					for (UINT i = 0; i < cTargets; ++i)
					{
						...
						{
							GetWindowRect(pTargets[i].hWnd, &r);
						}
						...
						for (const auto& oldObj : g_DrawObjects)
						{
							if (oldObj.hTargetWnd == pTargets[i].hWnd)
							{
								...
								updatedObjects[i].dwMoveMode = aeroflip::eWDOMM_MOVING_TO_BACK;
								bFoundOld = TRUE;
								break;
							}
						}
						...
						if (!bFoundOld)
						{
							...
							FLOAT w = (FLOAT)(r.right - r.left); if (w <= 0) w = 100.0f;
							FLOAT h = (FLOAT)(r.bottom - r.top); if (h <= 0) h = 100.0f;
...
void UpdateWindowAnimations(FLOAT fDeltaTime)
{
	...
	for (INT i = 0; i < iNumWindows; ++i)
	{
		...
		else if (iRelativeIndex == iNumWindows - 1 && iNumWindows > 1)
		{
